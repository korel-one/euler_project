#pragma once

/*The number, 1406357289, is a 0 to 9 pandigital number because it is made up of each of the 
digits 0 to 9 in some order, but it also has a rather interesting sub-string divisibility property.

Let d1 be the 1st digit, d2 be the 2nd digit, and so on. In this way, we note the following:

	d2d3d4=406 is divisible by 2
	d3d4d5=063 is divisible by 3
	d4d5d6=635 is divisible by 5
	d5d6d7=357 is divisible by 7
	d6d7d8=572 is divisible by 11
	d7d8d9=728 is divisible by 13
	d8d9d10=289 is divisible by 17

Find the sum of all 0 to 9 pandigital numbers with this property.*/

#include <array>
#include <algorithm>
#include <iostream>
#include <string>

bool sub_string_divisibility(const std::string& i_str) {
	int first = 1, sub_len = 3;

	const std::array<int, 7> divisor = {2, 3, 5, 7, 11, 13, 17};

	for (int i = 0; i <= 7; ++first, ++i) {
		auto curr = std::stoi(i_str.substr(first, sub_len));
		if (curr % divisor[i] != 0) {
			return false;
		}
	}

	return true;
}

void execute_43() {
	std::string pandigital = "1023456789";

	unsigned long long sum = 0;

	do
	{
		if (sub_string_divisibility(pandigital)) {
			unsigned long long number = 0; long long rank = 1;
			for (auto it = pandigital.rbegin(); it != pandigital.rend(); ++it) {
				if (*it != '0') {
					number += (*it - '0')* rank;
				}
				rank *= 10;
			}

			std::cout << pandigital << std::endl;
			std::cout << number << std::endl;
			std::cout << std::endl;
			sum += number;
			//sum += std::stoi(pandigital);
		}
	} while (std::next_permutation(pandigital.begin(), pandigital.end()));


	std::cout << "sum = " << sum << std::endl;
}